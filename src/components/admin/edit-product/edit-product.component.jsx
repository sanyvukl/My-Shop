...
import { UPDATE_PRODUCT } from "../../../redux/product/product.slice";
import { getStoreImagesUrl, getProductById, updateProductDocument } from "../../../utils/firebase/firebase.utils";
...
const EditProduct = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const productdData = await getProductById(params.productID);
            setProduct({ ...product, ...productdData });
        }
        fetchData();
        setIsLoading(false);
    }, [params.productID]);
    ...
    const updateProductHandler = async (product) => {
        try {
            await updateProductDocument(params.productID, product);
            dispatch(UPDATE_PRODUCT({ id: params.productID, data: product }))
    ...
    const onSubmitHandler = async (e) => {
        ...
        await updateProductHandler(newProduct);
    }
                ...
                <form onSubmit={onSubmitHandler}>
                    ...
                    <Card cardClass="group">
                        ...
                        <input
                            ...
                            value={product?.imageUrls ? product.imageUrls[0] : product.firstImageUrl}
                            required
                            disabled
                        />
                    </Card>
                    ...
                    <textarea name="desc" value={product.desc} cols="30" rows="10" onChange={handleChange} required></textarea>
                    {/*  */}
